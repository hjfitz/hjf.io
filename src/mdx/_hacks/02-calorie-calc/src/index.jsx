...
const Calculator = () => {
	...
	function calculate() {
		...
		const tdeeMod = {none: 1.2, light: 1.375, mod: 1.55, heavy: 1.725, vheavy: 1.9}[exercise]
		// todo: validation
		...
		setResults({
			bmr,
			tdee,
			cut: {20: tdee * 0.8, 15: tdee * 0.85, 10: tdee * 0.9},
			bulk: {20: tdee * 1.2, 15: tdee * 1.15, 10: tdee * 1.0},
		})
	}
			...
			<div className="flex flex-col sm:flex-row">
				<div>
					...
					<div>
						...
						<div className="grid gap-2">
							...
							<label>
								<input defaultChecked className="mr-2" type="radio" name="exercise" value="mod" onClick={clickSetExercise} />
				...
				<div className="flex flex-col">
					...
					<label className="my-1">
						...
						<input className="px-1 py-0.5 ml-2 border-2 border-blue-500 rounded-md w-36" ref={weightRef} onChange={calculate} defaultValue="85" type="number" />
					</label>
					<label className="my-1">
						<p className="inline">Height (cm)</p>
						<input className="px-1 py-0.5 ml-2 border-2 border-blue-500 rounded-md w-36" ref={heightRef} onChange={calculate} defaultValue="180" type="number" />
					</label>
					<label className="my-1">
						<p className="inline">Age (years)</p>
						<input className="px-1 py-0.5 ml-2 border-2 border-blue-500 rounded-md w-36" ref={ageRef} onChange={calculate} defaultValue="25" type="number" />
					</label>
				</div>
			</div>
			<div>
