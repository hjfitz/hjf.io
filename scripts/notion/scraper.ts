...
import {
	Frontmatter, RichText, TextForm,
} from './types'

const GENERATED_POST_ROOT = join(__dirname, '../../src/mdx/generated')
...
export default class NotionPage {
	...
	private get slug(): string {
		return this.title.replace(/[^\w-]+/g, ' ').trim().replace(/ /g, '-').toLowerCase()
	}
	...
	public parseBlocks() {
		const blocks = this.pageContents.results.map(async (block: any) => {
			...
			case 'paragraph': {
				...
				const content = NotionPage.parseRichText(block[blockType].rich_text)
	...
	public async resolveImage(url: string, src?: string): Promise<string> {
		...
		await new Promise((res) => createWriteStream(imgLocation).write(rawImg, res))
		return imgSrc
	}
	...
	public async createPage() {
		...
		// fetch cover image
		const featureSrc = await this.resolveImage(this.pageMeta.cover.external.url, 'feature')
		const postDescription = this.pageMeta.properties.Description.rich_text.map((p: any) => p.text.content).join('')
